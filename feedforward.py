...
class MakeCutouts(nn.Module):
    def __init__(self, cut_size, cutn, cut_pow=1.0, augs=None, pool=True):
        super().__init__()
        ...
        for item in augs:
            if item == "Ji2":
                augment_list.append(
                    K.ColorJitter(
                        brightness=0.1, contrast=0.1, saturation=0.05, hue=0.05, p=0.5
                    )
                )
            elif item == "Ji":
                ...
                augment_list.append(
                    K.RandomAffine(
                        degrees=15, translate=0.1, p=0.7, padding_mode="border"
                    )
                )
            elif item == "Et":
                ...
                augment_list.append(
                    K.RandomThinPlateSpline(scale=0.3, same_on_batch=False, p=0.7)
                )
            elif item == "Cr":
                augment_list.append(
                    K.RandomCrop(size=(self.cut_size, self.cut_size), p=0.5)
                ...
                augment_list.append(
                    K.RandomErasing(
                        (0.1, 0.4), (0.3, 1 / 0.3), same_on_batch=True, p=0.7
                    )
                )
            elif item == "Re":
                augment_list.append(
...
def encode_text_and_images(
    folder, *, img_ext="jpg", text_ext="txt", out="features.pkl", clip_model="ViT-B/32"
):
    """
...
def train(config_file, single_prompt=None):
    slug = mk_slug(single_prompt)
    ...
    if single_prompt:
        toks = load_dataset(None, text=single_prompt)
    else:
    ...
    vq_image_size = config.get(
        "vq_image_size", 16
    )  # if bigger, resolution of generated image is bigger
    ...
    make_cutouts = MakeCutouts(
        clip_size, cutn=cutn, augs=config.get("augs"), pool=config.get("pool", True)
    )
    ...
    dataloader = torch.utils.data.DataLoader(
        dataset, batch_size=bs, num_workers=0, sampler=sampler, shuffle=shuffle
    )
    ...
    for epoch in range(epochs):
        ...
        # has to be associated with the prompt somehow, maybe by path
        # but still use the same opt checkpoints and such
        for inp, out in dataloader:
            ...
            # bs,clip_dim
            inp_feats = (
                perceptor.encode_text(inp).float()
            ...
            # bs,clip_dim
            out_feats = (
                perceptor.encode_text(out).float()
                ...
                for feats in lpips.net((xr - mean) / std):
                    if diversity_mode == "between_same_prompts":
                        ...
                        div += (
                            (
                                (
                                    feats.view(repeat, 1, bs, cc, hh, ww)
                                    - feats.view(1, repeat, bs, cc, hh, ww)
                                )
                                ** 2
                            )
                            .sum(dim=3)
                        ...
                        div += (
                            (
                                (
                                    feats.view(nb, 1, cc, hh, ww)
                                    - feats.view(1, nb, cc, hh, ww)
                                )
                                ** 2
                            )
                            .sum(dim=2)
                        ...
                        raise ValueError(
                            "diversity_mode should be 'between_same_prompts' lr 'all'"
                        )
            else:
            ...
            if input_loss:
                ...
                # cutn*repeat*bs,clip_dim
                H = H.view(-1, clip_dim)
                dists += input_loss_coef * (
                    (H.sub(embed).norm(dim=-1).div(2).arcsin().pow(2).mul(2)).mean()
                )
            opt.zero_grad()
            ...
            if rank_zero and step % log_interval == 0:
                ...
                TF.to_pil_image(grid).save(
                    os.path.join(config.folder, f"progress_{step:010d}.png")
                )
                ...
                if inp.dtype == torch.long:
                    ...
                    with open(os.path.join(config.folder, "progress.txt"), "w") as fd:
                        fd.write(text)
                    with open(
                        os.path.join(config.folder, f"progress_{step:010d}.txt"), "w"
                    ) as fd:
                        fd.write(text)
                ...
                with torch.no_grad():
                    inp_feats = (
                        perceptor.encode_text(inp_fixed_batch).float()
                        if inp_fixed_batch.dtype == torch.long
                        else inp_fixed_batch.float()
                    )
                    if normalize_input:
                        inp_feats = F.normalize(inp_feats, dim=1)
                    if noise_dim:
                        inp_feats = torch.cat(
                            (inp_feats, noise[: len(inp_feats)]), dim=1
                        )
                    if use_ema:
                        with ema.average_parameters():
                            z = net(inp_feats)
                ...
                TF.to_pil_image(grid).save(
                    os.path.join(config.folder, "fixed_batch_progress.png")
                )
                # maybe config.folder / slug(single_prompt) / progress.png
                TF.to_pil_image(grid).save(
                    os.path.join(config.folder, f"fixed_batch_progress_{step:010d}.png")
                )
                if step == 0 and inp_fixed_batch.dtype == torch.long:
                    text = "\n".join([decode(t.tolist()) for t in inp_fixed_batch])
                    with open(
                        os.path.join(config.folder, "fixed_batch.txt"), "w"
                    ) as fd:
                        fd.write(text)

                if use_wandb:
                    caption = (
                        [decode(t.tolist()) for t in inp]
                        if inp.dtype == torch.long
                        else None
                    )
                    caption_fixed_batch = (
                        [decode(t.tolist()) for t in inp_fixed_batch]
                        if inp_fixed_batch.dtype == torch.long
                        else None
                    )
                    xr = xr.view(
                        repeat, bs, xr.shape[1], xr.shape[2], xr.shape[3]
                    ).cpu()
                    log = {}
                    log["image"] = [
                        wandb.Image(
                            xr[r, i].cpu(), caption=caption[i] if caption else None
                        )
                    ...
                    log["image_fixed"] = [
                        wandb.Image(
                            xr_fixed_batch[i].cpu(),
                            caption=caption_fixed_batch[i]
                            if caption_fixed_batch
                            else None,
                        )
                    ...
                    model_artifact = wandb.Artifact(
                        "trained-model", type="model", metadata=dict(net.config)
                    )
...
def test(
    ...
    *,
    ...
    with torch.no_grad():
        # this is great for for inference-only but i'd love to have a training loop here
        z = net(H)
        z = clamp_with_grad(z, z_min.min(), z_max.max())
        xr = synth(model, z)
    grid = torchvision.utils.make_grid(
        xr.cpu(), nrow=images_per_row if images_per_row else nb_repeats
    )
    ...
    *,
    ...
    for batch_idx, (tok,) in enumerate(dataloader):
        ...
        if save_images:
            ...
            TF.to_pil_image(grid).save(
                os.path.join(img_folder, f"batch_{batch_idx:010d}.png")
            )
            text = "\n".join([decode(t.tolist()) for t in tok])
            with open(
                os.path.join(img_folder, f"batch_{batch_idx:010d}.txt"), "w"
            ) as fd:
                fd.write(text)
        xr = torch.nn.functional.interpolate(
            xr, size=(clip_size, clip_size), mode="bilinear"
        )
        xr = (xr - mean) / std
    ...
    print(
        f"Fraction of images with a CLIP score of at least {clip_threshold}: {clip_score_atleast}"
    )


def load_dataset(path, text=None):
